...
intensities.
     ...
     -s     spot2Oligo filename (optional, default is ../../MITOMI/MITOMI_Motif/8MerLibrary/Spot2OligoFile080108)
...
def main(argv=None):
    ...
    concatFileName = ""
    spot2OligoFileName = "../../MITOMI/MITOMI_Motif/8MerLibrary/Spot2OligoFile_080108"
    pTh = 1
    DNATh = 1
    chTh = 1000
    DNAYMin = -1
    ...
    blockL,oRL,oCL,rows,cols,diaL,flagL,pFL,dFL,pBL,dBL,cFL = chipSingleconcUtils.concatFileToLists_v3(concatFileName)

    # determine spots per column and number of columns
    numCols,spotsPerCol = chipSingleconcUtils.determineDimensions_v3(concatFileName)
    ...
    flags,pFg,DNAFg,pBg,DNABg,chFg = chipSingleconcUtils.zeroFlaggedSpots(flagL,pFL,dFL,pBL,dBL,cFL)

    # create pBSub, DNABSub, and chBSub lists
    pBSub, DNABSub, chBSub = chipSingleconcUtils.backgroundSubtract_v2(pFL,pBL,dFL,dBL,cFL,dBL,pTh=pTh,dTh=DNATh,cTh=chTh)

    # create ratio and ratioNorm lists
    ratio, ratioNorm, ratioNormNorm, chBSubMean = chipSingleconcUtils.calculateRatios(pBSub,DNABSub,chBSub)
    ...
    # display the data
    listOfArrays = [DNAFgArray,DNABgArray,DNABSubArray,pFgArray,pBgArray,pBSubArray,chBSubArray,flagArray,oligoArray,ratioArray]
    listOfFilenames = ["DNAFgArray","DNABgArray","DNABSubArray","pFgArray","pBgArray","pBSubArray","chBSubArray","flagArray","oligoArray","ratioArray"]
    listOfColormaps = [1,1,1,2,2,2,1,1,1,1]
    for n in range(0,len(listOfArrays)):
        figFileRoot = analysisDir+listOfFilenames[n]
        plotUtils.createAndSaveHeatMap(listOfArrays[n],figFileRoot,xLabel="Columns",yLabel="Spots",xMax=0,xMin=numCols,yMax=0,yMin=spotsPerCol,colorMap=listOfColormaps[n],vMin=0,majorFontSize=16)
        
    # ok, now write out the arrays to text files to use for creating heat maps in igor.
    for n in range(0,len(listOfArrays)):
...
#    # now create an array to display DNABSub values side by side with oligo numbers
#    dimensions = (spotsPerCol,2*numCols)
#    DNABSubPlus,ratioPlus = N.zeros(dimensions),N.zeros(dimensions)
#    for n in range(0,len(DNABSub)):
#        DNABSubPlus[rows[n]-1][(cols[n]-1)*2]=DNABSub[n]
#        ratioPlus[rows[n]-1][(cols[n]-1)*2]=ratio[n]
#        spotIndex = str(cols[n])+"."+str(rows[n])
#        oligo = oligoD[spotIndex]
#        oligoArray[rows[n]-1][cols[n]-1]=int(oligo.split("_")[1])
#        DNABSubPlus[rows[n]-1][(cols[n]-1)*2+1]=int(oligo.split("_")[1]) 
#        ratioPlus[rows[n]-1][(cols[n]-1)*2+1]=int(oligo.split("_")[1])
#    listOfArrays = [DNABSubPlus,ratioPlus]
#    listOfFilenames = ["DNABSubPlus","ratioPlus"]
#    listOfColormaps = [1,3]
#    for n in range(0,len(listOfArrays)):
#        figFileRoot = analysisDir+listOfFilenames[n]
#        plotUtils.createAndSaveHeatMap(listOfArrays[n],figFileRoot,xLabel="Columns",yLabel="Spots",xMax=0,xMin=numCols,yMax=0,yMin=spotsPerCol,colorMap=listOfColormaps[n])

    # create graphs showing each intensity value versus position within each column (row)
    graphDataList = [rows,pFg,DNAFg,pBg,DNABg,chFg,pBSub,DNABSub,chBSub,ratio,ratioNorm]
    graphNameList = ["rows","pFg","DNAFg","pBg","DNABg","chFg","pBSub","DNABSub","chBSub","ratio","ratioNorm"]
    ...
    for n in range(1,len(graphDataList)):
        ...
        plotUtils.createAndSaveFig(graphDataList[0],graphDataList[n],xLabel=xLabel,yLabel=yLabel,figFileRoot=figFileName,yMin=yMinList[n])
        
#    colDir = analysisDir+"ByColumn/"
#    fileIOUtils.createNewDir(colDir)
#
#    # create text files and graphs with information by column
#    dataList = [pBSub,DNAFg,DNABSub,chBSub,ratio,ratioNorm]
#    dataNames = ['pBSub','DNAFg','DNABSub','chBSub','ratio','ratioNorm']
#    yLabels = ['Intensity','Intensity','Intensity','Intensity','Ratio','Normalized Ratio']
#    yMinList = [-1,DNAYMin,-1,-1,-1,-1]
#    fileNameRoots = []
#
#    for n in range(0,len(dataNames)):
#        fileNameRoots.append((colDir)+dataNames[n])
#
#    for n in range(0,len(dataList)):
#        chipSingleconcUtils.extractColumnData(concatFileName,fileNameRoots[n],analysisDir,rows,cols,dataList[n],dataNames[n],yLabels[n],yMinList[n])
