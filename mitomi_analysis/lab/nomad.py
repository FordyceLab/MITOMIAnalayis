...
# Classes for NOMAD
#
# 	$Id: nomad.py,v 1.25 2007/06/07 21:09:12 kael Exp $	
#
#
...
class nomadDB (kdbom.db):
    def __init__(self,host=defaulthost,cnfFilePath=None):
        if cnfFilePath == None:
            kdbom.db.__init__(self, host=defaulthost,user=defaultuser,
                              passwd=defaultpw,db='NOMAD',getTables=False)
        else:
            kdbom.db.__init__(self, db='NOMAD',host=host,
                              cnf=cnfFilePath,getTables=False)
        #
...
class result (kdbom.record):
    ...
    def spotCount(self):
        """Return the number of spots in NOMAD.
        """
        return self.dataTable.count({'Result_ID':self.resultID})
...
###                  pageposition = (300,1850),
    ...
    def GPR(self):
        ...
        try:
            ...
            pass
        try:
            rObj.optionalHeaders['Recreator'] = '$Id: nomad.py,v 1.25 2007/06/07 21:09:12 kael Exp $'
        except:
            pass
        try:
