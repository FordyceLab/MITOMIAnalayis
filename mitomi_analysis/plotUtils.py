...
def createAndSaveFig(xData,yData,figFileRoot,xLabel="",yLabel="",fileExt='.png',xMin=-1,xMax=-1,yMin=-10,yMax=-1,log2Y=0,log2X=0,plotType='bo',axisFontSize=20,tickFontSize=16,svgFlag=0):
    ...
    if log2Y == 1:
        ...
        if log2X == 0:
            tempPlot = plt.plot(xData,yData,plotType,hold="False",alpha=0.5)
        else:
            tempPlot = plt.semilogx(xData,yData,plotType,basey=2,hold='False')
...
def createAndSaveLogLogPlot(xData,yData,figFileRoot,xLabel="",yLabel="",fileExt='.png',xMin=-1,xMax=-1,yMin=-1,yMax=-1,plotType='bo',axisFontSize=20,tickFontSize=16,svgFlag=0):
...
def createAndSaveScatterPlot(xData,yData,xLabel="",yLabel="",figFileRoot="",fileExt='.png',xMin=0,xMax=-1,yMin=0,yMax=-1,svgFlag=0):
...
def createAndSaveScatterPlotXYErrors(xData,yData,xError,yError,xLabel,yLabel,figFileRoot,fileExt,svgFlag=0):
...
def createAndSaveBarChart(xData,yData,xLabel="",yLabel="",figFileRoot="",fileExt='.png',xMin=0,xMax=0,yMin=0,yMax=0):
...
def createAndSaveBarChartWithErrors(yData,yError,xLabel="",yLabel="",figFileRoot="",fileExt='.png',xMin=0,xMax=0,yMin=0,yMax=0):
...
def createAndSave2BarChart(yData1,yData2,xLabel="",yLabel="",figFileRoot="",fileExt='.png',xMin=0,xMax=0):
...
def createAndSave2BarChartWithErrors(yData1,yData2,yError1,yError2,xLabel,yLabel,figFileRoot,fileExt):
...
def createAndSaveMultitraceFig(pairedDataList,xLabel="",yLabel="",figFileRoot="",fileExt='.png'):
...
def newCreateAndSaveMultitraceFig(xDataList,yDataList,xLabel="",yLabel="",figFileRoot="",fileExt='.png',xMin=-10,xMax=0,yMin=-10,yMax=-1,nameList=[]):
    ...
    for n in range(0,len(xDataList)):
        ...
        tempPlot = plt.plot(xData,yData,plotDict[str(n+1)],hold="True",alpha=0.5)
    ...
    plt.ylim(yMin,yMax)
    #plt.legend(nameList)
    #leg = newLegend(nameList,loc='outer right')
    #leg.draw_frame(False)
    plt.savefig(figFileName,dpi=150)
    plt.clf()


def createAndSaveMultilineFig(pairedDataList,xLabel="",yLabel="",figFileRoot="",fileExt='.png',xMin=0,xMax=0):
...
def newCreateAndSaveMultilineFig(xDataList,yDataList,xLabel="",yLabel="",figFileRoot="",fileExt='.png',xMin=0,xMax=0,yMin=0,yMax=0,legendFlag=1,legendFont=12,traceNameList=[],legLoc=(0,0)):
    """This subroutine saves a figure with multiple lines."""
    ...
    for n in range(0,len(xDataList)):
        xData = convert_list_to_array(xDataList[n])
        yData = convert_list_to_array(yDataList[n])
        tempPlot = plt.plot(xData,yData,colorDict[str(n+1)],hold="True",label=traceNameList[n])
...
def createAndSaveMultilineFigSemilogX(pairedDataList,xLabel,yLabel,figFileRoot,fileExt):
    """This subroutine saves a figure with multiple lines."""
    ...
    for n in range(0,len(pairedDataList)):
        xData = convert_list_to_array(pairedDataList[n][0])
        yData = convert_list_to_array(pairedDataList[n][1])
        tempPlot = semilogx(xData,yData,colorDict[str(n+1)],basex=2,hold="True")
...
def createAndSaveMultilineFigSemilog10X(pairedDataList,xLabel,yLabel,figFileRoot,fileExt):
    """This subroutine saves a figure with multiple lines."""
...
def create_multiple_trace_dict():
    ...
    markerList = ['o','s','D','v','+','x','^','>','<','p','1','2','3','4']
    symbolList = []
    for m in range(0,14):
            marker = markerList[m]
            for l in range(0,7):
...
def convert_list_to_array(inData):
    ...
    if isinstance(inData,list)==True:
        inData = array(inData)
...
def append_multiple_traces_to_graph(yValuesList,xValuesList):
    ...
    if len(xValuesList)!=listLen:
        ...
        for n in range(0,listLen):
            ...
            tempPlot = plot(xValuesList[n],yValuesList[n],plotArgDict[curColorIndex],hold=True)
    hold=False
    return tempPlot
...
def gnuplotScriptManyFilesOneGraph(scriptName,fileList,title,xLabel,yLabel,graphName):
    ...
    for n in range(0,(len(fileList)-1)):
        plotCommand = plotCommand+str(fileList[n])+' with points lt '+str(n+1)+' pt 7 ps 2, '
    plotCommand = plotCommand+str(fileList(len(fileList)))+' with points lt '+str(len(fileList)+1)+' pt 7 ps 2\n'
...
def makeHist(dataIn,figFileRoot,numBins=None,normed=0,facecolor='green',xLabel='',yLabel='',log=False,xMin=-10,xMax=-10,yMin=-10,histtype='bar',removeNaNFlag=0,axisFontSize=22,tickFontSize=18,svgFlag=0):
    if removeNaNFlag == 1:
        newData = []
        for item in dataIn:
            if isnan(item) == False:
    ...
    if xMax == -10:
        xMax = max(dataIn)
    if numBins == None:
        n,bins,patches = plt.hist(dataIn,range=(xMin,xMax),normed=normed,log=log,facecolor=facecolor,histtype=histtype)
    else:
        n,bins,patches = plt.hist(dataIn,bins=numBins,range=(xMin,xMax),normed=normed,log=log,facecolor=facecolor,histtype=histtype)
    plt.xlabel(xLabel,fontsize=axisFontSize)
    plt.ylabel(yLabel,fontsize=axisFontSize)
    ax = plt.gca()
    for tick in ax.xaxis.get_major_ticks():
...
def overlapHist(dataA,dataB,figFileRoot,numBinsA=None,numBinsB=None,normed=0,facecolorA='green',facecolorB='0.8',xLabel='',yLabel='',log=False,xMin=-10,xMax=-10,yMin=-10,histtype='bar',removeNaNFlag=0,axisFontSize=22,tickFontSize=18):
    if removeNaNFlag == 1:
        newDataA,newDataB = [],[]
        for item in dataA:
            if isnan(item) == False:
    ...
    if xMax == -10:
        ...
        if max(dataB) > xMax:
            xMax = max(dataB)
    if numBinsA == None and numBinsB == None:
        nA,binsA,patchesA = plt.hist(dataA,range=(xMin,xMax),normed=normed,log=log,facecolor=facecolorA,histtype=histtype,hold=True)
        nB,binsB,patchesB = plt.hist(dataB,range=(xMin,xMax),normed=normed,log=log,facecolor=facecolorB,histtype=histtype)
    else:
        nA,binsA,patchesA = plt.hist(dataA,bins=numBinsA,range=(xMin,xMax),normed=normed,log=log,facecolor=facecolorA,histtype=histtype,hold=True)
        nB,binsB,patchesB = plt.hist(dataB,bins=numBinsB,range=(xMin,xMax),normed=normed,log=log,facecolor=facecolorB,histtype=histtype)
    plt.xlabel(xLabel,fontsize=axisFontSize)
    plt.ylabel(yLabel,fontsize=axisFontSize)
    ax = plt.gca()
    for tick in ax.xaxis.get_major_ticks():
...
def createAndSaveHeatMap(dataArray,figFileRoot,xLabel="",yLabel="",xMin=0,xMax=-1,yMin=0,yMax=-1,colorMap=0,maxInt=0,vMin=0,vMax=0,fontSize=20,majorFontSize=18,pngDPI=150,svgDPI=75,svgFlag=0):
...
def createSingleFigWithSubplots(yDataList,figFileRoot,xLabel="",yLabel="",fileExt='.png',xMin=-1,xMax=-1,yMin=-1,yMax=-1,log2Y=0,log2X=0):
...
def newLegend(*args, **kwargs):
    """
602     Overwrites the pylab legend function.
603 
604     It adds another location identifier 'outer right'
605     which locates the legend on the right side of the plot
606 
607     The args and kwargs are forwarded to the pylab legend function
608     """
    ...
    if kwargs.has_key('loc'):
        ...
        if loc[0] == 'outer':
            ...
            if loc[1] == 'right':
                # scale the plot
                currentAxes.set_position((currentAxesPos[0],currentAxesPos[1], currentAxesPos[2] * (1-plotScaling),currentAxesPos[3]))
...
def createAndSaveRawAndMeanData(xDataRaw,yDataRaw,xDataMean,yDataMean,yDataSem,xLabel="",yLabel="",figFileRoot="",fileExt='.png',xMin=-100,xMax=0,yMin=-100,yMax=0):
    ...
    tempPlot = plt.plot(xDataRaw,yDataRaw,ls='None',marker='o',markersize=1.5,color='1.0',hold="True")
...
def createAndSaveRawAndMeanDataWLabels(xDataRaw,yDataRaw,xDataMean,yDataMean,yDataSem,xTicks,xLabels,xLabel="",yLabel="",figFileRoot="",fileExt='.png',xMin=-1,xMax=0,yMin=-100,yMax=0):
...
def createAndSaveContourPlot(zData,xData,yData,figFileName,xLabel,yLabel,zLabel):
...
def createAndSaveContourPlotExtraD(zData,xData,yData,colorData,figFileName,xLabel,yLabel,zLabel):
    ...
    ax = Axes3D(fig)
#    ax.plot3D(ravel(xA), ravel(yA), ravel(zA), c='b', marker='o', alpha=0.5)
#    ax.scatter3D(ravel(xA), ravel(yA), ravel(zA), c='r' ,marker='o', alpha=0.5)
#    ax.scatter3D(ravel(xA), ravel(yA), ravel(zA), cmap=plt.cm.Greens ,marker='o', alpha=0.5)
...
#def createAndSaveRawAndMeanDataOneCol(rawDataA,rawDataB,meanA,meanB,semA,semB,xTicks,xLabels,xLabel="",yLabel="",figFileRoot="",fileExt='.png',xMin=-1,xMax=0,yMin=-100,yMax=0):
#
#    figFileName = figFileRoot+fileExt
#
#    if xMax == 0:
...
#        
#    #axes().set_aspect('equal')
#    tempPlot = plot(xDataRaw,yDataRaw,ls='None',marker='o',markersize=1.5,color='1.0',hold="True")
#    tempPlot = errorbar(xDataMean,yDataMean,yerr=yDataSem,fmt='ro')
#
#    xlabel(xLabel,size=20)
#    ylabel(yLabel,size=20)
#    xlim(xMin,xMax)
#    ylim(yMin,yMax)
#    xticks(xTicks,xLabels,size='x-large')
#    savefig(figFileName,dpi=150)
#    figFileName = figFileRoot+'.svg'
#    savefig(figFileName,dpi=150)
#    clf()
