...
def criticalValDict():
    ...
    onePercentD = {1:318.313,2:22.327,3:10.215,4:7.173,5:5.893,6:5.208,7:4.782,8:4.499,9:4.296,10:4.143,11:4.024,12:3.929,13:3.852,14:3.787}
    fivePercentD = {1:63.657,2:9.925,3:5.841,4:4.604,5:4.032,6:3.707,7:3.499,8:3.355,9:3.250,10:3.169,11:3.106,12:3.055,13:3.012,14:2.977}
...
def checkForOutlier(value,avg,sdev,numPoints,percent):
    ...
    if percent == 1:
        ...
        print "You have chosen to use an invalid statistical cutoff for your t-test!"
...
def removeOutliers(inList):
    ...
    for n in range(0,len(inList)):
        ...
        if sdev != 0:
            ...
            if N.isnan(inList[n]) == False:
                qualityScore = checkForOutlier(inList[n],avg,sdev,numPoints,5)
                if qualityScore == 1:
                    print "old then new"
                    print inList
                    inList[n] = N.nan
...
def calcMeanNoNaNNoInf(inList):
    ...
    tempList = []
    for item in inList:
        if N.isnan(item) == True or N.isinf(item) == True:
            pass
        else:
            tempList.append(item)
...
def calcMedNoNaNNoInf(inList):
    ...
    newList = []
    for item in inList:
        if N.isnan(item) == False and N.isinf(item) == False:
            newList.append(item)
...
def calcGeneListOverlapProb(n,m,n1,n2):
    """This function usess a hypergeometric probability distribution to calculate the probability of obtaining
    a number of overlapping genes (m) between 2 lists of length n1 and n2 out of a total set of n genes."""
