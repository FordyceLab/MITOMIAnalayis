...
3.  a processed concat file with oligo numbers and sequences assigned to each spot

Optional threshhold values can be given to set a lower bound on permissible background
subtracted intensities.

     -h     print this help message
     -c     concat filename (required)
     -s     spot2oligo filename (optional, default is /Users/pollyfordyce/Documents/lib/perl/PR8MerSpot2OligoFile.txt)
     ...
     -f     filename containing oligo numbers and sequences (default = /Users/pollyfordyce/Documents/lib/perl/8MerLib.txt)
...
def main(argv=None):
    ...
    concatFileName = ""
    spot2OligoFileName = "/Users/pollyfordyce/Documents/lib/perl/PR8MerSpot2OligoFile.txt"
    pTh = -500
    DNATh = -500
    chTh = -1000
    nanFlag = 0
    ...
    blocks,oRows,oCols,rows,cols,flags,pFg,DNAFg,pBg,DNABg,chFg,pBSub,DNABSub,chBSub,ratio,ratioNorm,oligoNum = chipSingleconcUtils.outputInfoFromConcatFile_v3(concatFileName, spot2OligoFileName, pTh=pTh, DNATh=DNATh, chTh=chTh, nanFlag=nanFlag)
    
    # make a dictionary linking oligo number and sequence
    oligoSeqD = chipSingleconcUtils.createDictFromSeqFile(oligoSeqFileName, truncFlag=truncFlag)
    ...
    # normalize waves so that they are centered around zero, normalize ratio to a max of 1
    DNAN = chipSingleconcUtils.normalizeValues_v2(DNABSub,analysisDir,"DNABSub.png",inHi=0,numBins=100)
    pN = chipSingleconcUtils.normalizeValues_v2(pBSub,analysisDir,"pBSub.png",inHi=0,numBins=100)
    rN = chipSingleconcUtils.normalizeValues_v2(ratio,analysisDir,"ratio.png",inHi=0,numBins=100)    
    ...
    dParams = fitUtils.gaussianFit(DNAN,numBins=100,figFileName=checkDir+'DNAN.png',loBound=0,hiBound=0)
    pParams = fitUtils.gaussianFit(pN,numBins=100,loBound=0,hiBound=0,figFileName=checkDir+'pN.png')
    rParams = fitUtils.gaussianFit(rNN,numBins=100,loBound=0,hiBound=0,figFileName=checkDir+'rNN.png')
    ...
    outFile.write('Block\tOrigRow\tOrigCol\tRow\tCol\tFlag\tpFg\tDNAFg\tpBg\tDNABg\tchFg\tpBSub\tDNABSub\tchBSub\tRatio\tRatioNorm\tOligoNum\tOligoSeq\tDNAN\trN\trNN\tZScore\tpVal\n')
    outLists = [blocks,oRows,oCols,rows,cols,flags,pFg,DNAFg,pBg,DNABg,chFg,pBSub,DNABSub,chBSub,ratio,ratioNorm,oligoNum,oligoSeq,DNAN,rN,rNN,zScore,pVal]
    ...
    numSpotsPerOligo = chipSingleconcUtils.calcNumSpotsPerOligo(spot2OligoFileName)
    ...
    chBSubArray = chipSingleconcUtils.dataArray(spot2OligoFileName,cols,rows,chBSub)
    colArray = chipSingleconcUtils.dataArray(spot2OligoFileName, cols, rows, cols)
    rowArray = chipSingleconcUtils.dataArray(spot2OligoFileName, cols, rows, rows)
    DNANArray = chipSingleconcUtils.dataArray(spot2OligoFileName, cols, rows, DNAN)
    pBSubArray = chipSingleconcUtils.dataArray(spot2OligoFileName, cols, rows, pBSub)
    rNNArray = chipSingleconcUtils.dataArray(spot2OligoFileName, cols, rows, rNN)
    ...
    for n in range(0,numOligos):
        ...
        for m in range(0,len(rNNByOligo)):
            writeList = [str(repNum[m]+1),'\t',str(pBSubByOligo[m]),'\t',str(DNANByOligo[m]),'\t',str(rNNByOligo[m]),'\t',str(chBSubByOligo[m]),'\t',str(rowByOligo[m]),'\t',str(colByOligo[m]),'\n']
    ...
    plotUtils.createAndSaveFig(oligoNumPlotting,DNANPlotting,figFileRoot,xLabel="Oligo",yLabel="Intensity",yMin=0,xMin=0,xMax=numOligos)
    figFileRoot = graphDir+"rNNVsOligo"
    plotUtils.createAndSaveFig(oligoNumPlotting,rNNPlotting,figFileRoot,xLabel="Oligo",yLabel="Ratio",yMin=0,xMin=0,xMax=numOligos)
    
    # now go through and make histograms of all DNABSub, Ratio, and RatioNorm values
    figFileRoot = graphDir+'/DNANHist'
    plotUtils.makeHist(DNAN,figFileRoot,numBins=1000,xLabel='DNAN',yLabel='Number of Events',log=False,removeNaNFlag=1)
    figFileRoot = graphDir+'/DNANHistLog'
    plotUtils.makeHist(DNAN,figFileRoot,numBins=1000,xLabel='DNAN',yLabel='Number of Events',log=True,removeNaNFlag=1)
    figFileRoot = graphDir+'/rNNHist'
    plotUtils.makeHist(rNN,figFileRoot,numBins=1000,xLabel='rNN',yLabel='Number of Events',log=False,removeNaNFlag=1)
    figFileRoot = graphDir+'/rNNHistLog'
    plotUtils.makeHist(rNN,figFileRoot,numBins=1000,xLabel='rNN',yLabel='Number of Events',log=True,removeNaNFlag=1)
    return 0
